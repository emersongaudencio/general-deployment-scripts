...
if [[ $os_type == "rhel" ]]; then
    if [[ $os_version == "7" ]]; then
      ...
      $(type -p dnf || type -p yum) -y install http://rpms.remirepo.net/enterprise/remi-release-7.rpm
    elif [[ $os_version == "8" ]]; then
      # -------------- For RHEL/CentOS 8 --------------
      $(type -p dnf || type -p yum) -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
      $(type -p dnf || type -p yum) -y install http://rpms.remirepo.net/enterprise/remi-release-8.rpm
    fi
fi

verify_redis=`rpm -qa | grep redis`
if [[ $verify_redis == "redis"* ]]
then
echo "$verify_redis is installed!"
else
   ##### FIREWALLD DISABLE #########################
   ...
   if [[ $os_version == "7" ]]; then
     # -------------- For RHEL/CentOS 7 --------------
     $(type -p dnf || type -p yum) -y --enablerepo=remi install redis
   elif [[ $os_version == "8" ]]; then
     # -------------- For RHEL/CentOS 8 --------------
     dnf module reset redis -y
     dnf module install redis:remi-7.0 -y
   fi

   ##### CONFIG PROFILE #############
   check_profile=$(cat /etc/profile | grep '# redis-pre-reqs' | wc -l)
   if [ "$check_profile" == "0" ]; then
   echo ' ' >> /etc/profile
   echo '# redis-pre-reqs' >> /etc/profile
   echo 'if [ $USER = "redis" ]; then' >> /etc/profile
   ...
   echo "Redis Pre-reqs for /etc/profile is already in place!"
   fi

   #####  Redis LIMITS ###########################
   check_limits=$(cat /etc/security/limits.conf | grep '# redis-pre-reqs' | wc -l)
   if [ "$check_limits" == "0" ]; then
   echo ' ' >> /etc/security/limits.conf
   echo '# redis-pre-reqs' >> /etc/security/limits.conf
   echo 'redis              soft    nproc   102400' >> /etc/security/limits.conf
   echo 'redis              hard    nproc   102400' >> /etc/security/limits.conf
   echo 'redis              soft    nofile  102400' >> /etc/security/limits.conf
   echo 'redis              hard    nofile  102400' >> /etc/security/limits.conf
   echo 'redis              soft    stack   102400' >> /etc/security/limits.conf
   ...
   echo "Redis Pre-reqs for /etc/security/limits.conf is already in place!"
   fi

   ##### Redis SYSCTL ###########################
   check_sysctl=$(cat /etc/sysctl.conf | grep '# redis-pre-reqs' | wc -l)
...
net.core.wmem_max=1048586" >> /etc/sysctl.conf
   ...
   echo "Redis Pre-reqs for /etc/sysctl.conf is already in place!"
   fi
   ...
   echo 'LimitNOFILE=102400' >> /etc/systemd/system/redis.service.d/limits.conf
